...
def calculate_score(bid_card, diamond_card, user):
    """
    Calculate the score based on the selected bid and diamond card.
    
    Args:
        bid_card (Card): The card bid by the player or computer.
        diamond_card (Card): The selected diamond card for the current round.
        user (Player): The player object.
    
    Returns:
        float: The score calculated based on bid and diamond cards.
    """
    if bid_card.rank > diamond_card.rank:
        return bid_card.rank + diamond_card.rank
    elif bid_card.rank < diamond_card.rank:
        return -bid_card.rank
    else:
        return 0
